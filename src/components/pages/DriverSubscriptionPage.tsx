...
export default function DriverSubscriptionPage({ onNavigate }: DriverSubscriptionPageProps) {
  ...
  useEffect(() => {
    ...
    if (role === 'admin') { setGate('approved'); return; }
  ...
  function handleSubscribeClick() {
    ...
    switch (gate) {
      case 'not-applied': onNavigate('driver-register'); return;
      case 'pending':
      case 'rejected':    onNavigate('driver-dashboard'); return;
      case 'approved':    onNavigate('driver-dashboard'); return;
      default:            return;
    }
  }

  function ctaLabel(planPrice: number): string {
    if (!user)                     return 'Sign in to apply';
    if (gate === 'loading')        return 'Checking…';
    if (gate === 'not-applied')    return 'Apply first';
    if (gate === 'pending')        return 'Under review';
    if (gate === 'rejected')       return 'Re-submit application';
    return planPrice === 0 ? 'Get Started Free' : 'Subscribe Now';
  }
          ...
          {SUBSCRIPTION_PLANS.map(plan => (
            <div
              ...
              className={`relative bg-white rounded-2xl p-6 flex flex-col transition-all ${
                plan.popular
                  ? 'border-2 border-[#F5B400] shadow-xl shadow-[#F5B400]/10 scale-[1.03] z-10'
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {SUBSCRIPTION_PLANS.map(plan => {
                  ...
                  return (
                    <tr key={plan.id} className={`border-b border-gray-50 hover:bg-gray-50 ${plan.popular ? 'bg-[#F5B400]/5' : ''}`}>
                      <td className="py-3 px-4 font-semibold text-[#0B2239]">{plan.name}</td>
                      <td className="py-3 px-4 text-gray-600">{calc.rate}%</td>
                      <td className="py-3 px-4 text-gray-500">£{calc.commission}</td>
                      <td className="py-3 px-4">
                        <span className={`font-black text-base ${plan.popular ? 'text-[#F5B400]' : 'text-[#0E2A47]'}`}>
                          £{calc.earning}
                        </span>
                      </td>
                    </tr>
