...
const BookingCard: React.FC<BookingCardProps> = ({ booking, onTrack, onPay, onConfirmDelivery }) => {
  ...
  return (
    <div className={`group bg-white rounded-3xl border transition-all hover:-translate-y-0.5 hover:shadow-lg ${isActive ? 'border-[#0E2A47]/20 shadow-md shadow-[#0E2A47]/5' : 'border-gray-100 shadow-sm'}`}>
      {/* Card top stripe for active bookings */}
      {isActive && (
        <div className="h-1 bg-gradient-to-r from-[#0E2A47] to-[#0F3558] rounded-t-3xl" />
      )}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          ...
          <div className="text-right shrink-0">
            <p className="text-2xl font-black text-[#0E2A47]">£{booking.estimated_price}</p>
        ...
        {booking.driver && (
          <div className="flex items-center gap-2 mb-4 py-2.5 px-3.5 bg-indigo-50 rounded-xl border border-indigo-100">
            <div className="w-6 h-6 bg-[#0E2A47] rounded-full flex items-center justify-center shrink-0">
        ...
        <div className="flex items-center justify-between gap-2 flex-wrap">
          ...
          <div className="flex gap-2 flex-wrap">
            {showPayBtn && (
              <button
                onClick={onPay}
                className="flex items-center gap-1.5 bg-[#F5B400] hover:bg-[#E5A000] text-[#071A2F] px-4 py-2 rounded-xl text-xs font-black transition-all hover:shadow-md"
              >
            ...
            {isActive && (
              <button
                onClick={onTrack}
                className="flex items-center gap-1.5 bg-[#0E2A47] hover:bg-[#0F3558] text-white px-4 py-2 rounded-xl text-xs font-bold transition-colors"
              >
        ...
        <div className="py-8">
          <div className="flex items-start justify-between gap-4">
            ...
            <button
              ...
              className="flex items-center gap-2 text-gray-500 hover:text-[#0E2A47] font-medium text-sm transition-colors group shrink-0 mt-2"
            >
              ...
              {[
                ...
                <div key={i} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-3">
                  <div className="w-9 h-9 bg-[#0E2A47]/8 rounded-xl flex items-center justify-center shrink-0">
                    <stat.icon className="w-4 h-4 text-[#0E2A47]" />
                  </div>
                  <div>
          ...
          {(['active', 'history'] as const).map((t) => (
            <button
              ...
              className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${
                tab === t ? 'bg-white text-[#0E2A47] shadow-sm' : 'text-gray-500 hover:text-gray-700'
              }`}
            >
              {t === 'active' ? 'Active' : 'History'}
              {t === 'active' && active.length > 0 && (
                <span className="ml-2 bg-[#0E2A47] text-white text-xs rounded-full px-1.5 py-0.5 font-black">{active.length}</span>
              )}
            </button>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24">
            ...
            {tab === 'active' && (
              <button
                onClick={() => onNavigate('home')}
                className="bg-[#F5B400] hover:bg-[#E5A000] text-[#071A2F] px-8 py-3.5 rounded-2xl font-black text-sm transition-all hover:shadow-xl hover:shadow-[#F5B400]/25"
              >
                Get a Quote Now
              </button>
            )}
