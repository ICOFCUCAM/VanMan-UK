...
const NationwideCoverageStrip: React.FC = () => (
  <section
    className="py-10 border-t border-white/[0.05]"
    style={{ background: 'linear-gradient(180deg, #0E2A47 0%, #071A2F 100%)' }}
  >
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
      <div className="flex items-center justify-center gap-2 mb-3">
        <MapPin className="w-4 h-4 text-[#F5B400]" />
        <span className="text-[#F5B400] text-xs font-bold tracking-[0.2em] uppercase">Nationwide Network</span>
      </div>
      ...
      <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 max-w-2xl mx-auto">
        {cities.map((city) => (
          <span key={city} className="text-[#8FA9C4] text-xs font-medium">{city}</span>
        ))}
        <span className="text-[#F5B400] text-xs font-bold">+ many more</span>
      </div>
    </div>
  </section>
);
