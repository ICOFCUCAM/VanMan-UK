...
const SubscriptionTeaser: React.FC<SubscriptionTeaserProps> = ({ onNavigate }) => {
  const { user, role, isAuthenticated } = useAppContext();
  ...
  function handlePlanClick() {
    ...
    if (role === 'driver') {
      onNavigate('driver-subscription');
    } else {
  ...
  function ctaLabel(planPrice: number): string {
    ...
    if (role === 'driver') return planPrice === 0 ? 'Get Started Free' : 'View Plan';
    return 'Apply as a Driver';
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 max-w-2xl mx-auto text-base">
            All drivers start on Silver. Upgrade anytime to reduce your commission rate, unlock priority dispatch, and grow your earnings.
          </p>
          {!isAuthenticated && (
            <p className="mt-3 text-sm text-[#0E2A47] font-semibold">
              Already a verified driver?{' '}
              <button onClick={() => onNavigate('login')} className="underline hover:no-underline">
